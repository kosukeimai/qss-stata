...
*************************************************************
*************************************************************
twoway pcarrowi 0 -1 0 1  0 1 0 -1 , lcolor(black) mcolor(black) || ///
	pcarrowi  -1 0 1 0  1 0 -1 0 , lcolor(black) mcolor(black) || ///
	scatteri -.75 -.35, msymbol(T ) msize(large) || scatteri -.1 .3, ///
	msymbol(X) msize(huge) || scatteri .5 .5, msize(large) || ///
	scatteri -.75 -.35 -.1 .3, msymbol(i) connect(line) lpattern(dot) lcolor(black) || ///
	scatteri -.1 .3 .5 .5, msymbol(i) connect(line) lpattern(dash) lcolor(black) ///
	legend(off) text(.6 .6 "Status quo") text(-.9 -.35 "Proposal", place(9)) ///
	text(-.2 .4 "Ideal point") text(.15 .75 "Economic conservatism", place(0)) ///
	text(-.15 -.75 "Economic liberalism", place(0)) ///
	text(.9 .27 "Racial conservatism", place(0)) text(-.9 .25 "Racial liberalism", place(0))
...
* load the data
use congress, clear
* scatter plot for the 80th and 112th Congresses
